...
export default {
  ...
  getcrawlerStatus (ojname, userid) {
    return ajax('crawler/status', 'get', {
      params: {
        oj_name: ojname,
        user_id: userid
      }
  ...
  StudentJoinClass (classid, stuid, invitationnumber) {
    // 学生加入某个班级
  ...
  getClassesContestList (classid) {
    // 查询某个班级所有比赛
    return ajax('classes/contest', 'get', {
      params: {
        class_id: classid
      }
    })
  ...
  getMyHomework (classstatus, contestsstatus, offsets, limits, keywords) {
    return ajax('classes/contests', 'get', {
      params: {
        class_status: classstatus,
        contests_status: contestsstatus,
        offset: offsets,
        limit: limits,
        keyword: keywords
      }
...
function ajax (url, method, options) {
  ...
  return new Promise((resolve, reject) => {
    axios({
      ...
      params,
      data
      // 其中只有url参数是必须的，method默认为get方法，
      // params 把参数拼接到URL上 data 把参数放在请求体里面
    }).then(res => {
