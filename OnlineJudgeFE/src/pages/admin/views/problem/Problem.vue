    ...
    <Panel :title="title">
      <el-form ref="form" :model="problem" :rules="rules" label-position="top" label-width="70px">
        ...
        <el-form-item :label="$t('m.Source')">
          <el-input type="text" :placeholder="$t('m.Source')" v-model="problem.source"></el-input>
        </el-form-item>
        ...
        <el-row :gutter="20">
          <el-col :span="8">
            <el-form-item :label='"ProblemID"'>
              <el-input type="text" :placeholder='"ProblemID"' v-model="proid"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item :label='"sourceOJ"'>
              <el-select class="OJ-select" size="small" :placeholder='"sourceOJ"' v-model="ojname">
        ...
        <el-button type="primary" round size="small" @click="runclawer()">爬取</el-button>
      </el-form>
    </Panel>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      runclawer () {
        api.getcrawler(this.ojname, this.proid).then(res => {
          this.problem.title = res.data.data.title.toString()
          this.problem.description = res.data.data.description.toString()
          this.problem.input_description = res.data.data.Input.toString()
          this.problem.output_description = res.data.data.output.toString()
          this.problem.samples[0].input = res.data.data.sample_input.toString()
          this.problem.samples[0].output = res.data.data.samp_output.toString()
          this.problem.hint = res.data.data.hint.toString()
          this.problem.time_limit = parseInt(res.data.data.time.toString())
          this.problem.memory_limit = parseInt(res.data.data.memory.toString()) / 1000
          this.problem.oj = this.ojname + '|' + this.proid
          this.problem.source = this.ojname
          this.problem._id = this.problem.oj
          this.problem.tags.push(this.ojname)
          this.problem.test_case_id = 0
        }).catch(() => {
        })
      },
      ...
      submit () {
        ...
        if (!this.testCaseUploaded && !this.problem.oj) {
